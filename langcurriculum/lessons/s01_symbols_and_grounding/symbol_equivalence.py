"""Lesson 2: ``symbol_equivalence`` — many-to-one lexical semantics.
...
from ..._support.base import COLORS, _scene


def gen_symbol_equivalence(rng: random.Random):
    """Two symbols denote the same thing; the alias is defined *in the episode*."""
    objs = _scene(rng, rng.randint(3, 4))
    tgt = rng.choice(objs)
